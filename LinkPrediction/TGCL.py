...
class TGCL(nn.Module):
    def __init__(self, gnn, percept_gnn, args):
    ...
    def forward(self, x, edge_index, batch):
        rep = self.gnn(x, edge_index)
        feats = self.percept_gnn(x, edge_index)
        graph_rep = self.pool(rep, batch)
        scores = self.score_func(graph_rep)
        return graph_rep, scores, feats
...
def train(args, loader, model, optimizer, device):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        graph_rep, scores, feats = model(batch.x, batch.edge_index, batch.batch)
...
def main():
    ...
    parser.add_argument('--global_epochs', type=int, default=100,
                        help='Checkpoint Number of First Model (default: 50)')
    ...
    args.output_model_dir = args.output_model_dir + \
        '{}/TGCL_tau{}/'.format(args.dataset, TAU)
    ...
    #set up dataset
    dataset = TUDatasetExt("./linkPrediction_TU/datasets/" + args.dataset, args.dataset)
    ...
    dataset.transform = TGCLPerturbation(num_node_type, args)
    ...
    percept_gnn = GNN(args.num_layer, args.emb_dim, num_node_type, JK=args.percept_JK, 
                      drop_ratio=args.dropout_ratio, gnn_type='gcn')
    
    checkpoint = torch.load(
        f'./ckpts/{args.dataset}/DSLA/{args.global_epochs}.pth'
    )
    percept_gnn.load_state_dict(checkpoint)

    model = TGCL(gnn, percept_gnn, args)
    model.to(device)
