...
def scaffold_split_ours(dataset, smiles_list, task_idx=None, null_value=0, frac_train=0.8, frac_valid=0.2, frac_test=0.1, return_smiles=False):
    ...
    for scaffold_set in all_scaffold_sets:
        if len(train_idx) + len(valid_idx) + len(scaffold_set) <= valid_cutoff:
            #if np.random.uniform(0,1) > frac_train:
            #    valid_idx.extend(scaffold_set)
            #else:
            train_idx.extend(scaffold_set)
        else:
    ...
    train_dataset = dataset[torch.tensor(train_idx)]
    valid_dataset = dataset[torch.tensor(test_idx)]
    test_dataset = dataset[torch.tensor(test_idx)]
    ...
    if not return_smiles:
        ...
        train_smiles = [smiles_list[i][1] for i in train_idx]
        valid_smiles = [smiles_list[i][1] for i in test_idx]
        test_smiles = [smiles_list[i][1] for i in test_idx]
        return train_dataset, valid_dataset, test_dataset, (train_smiles,
                                                            valid_smiles,
                                                            test_smiles)
